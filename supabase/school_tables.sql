-- ============================================================
-- The Hungry Rooster — School Coffee Shop Account System
...
CREATE TABLE IF NOT EXISTS school_accounts (
  ...
  student_pin              TEXT UNIQUE NOT NULL,   -- 4–6 digit code typed at POS
  grade_class              TEXT,
  ...
  billing_preference       TEXT DEFAULT 'invoice'
                             CHECK (billing_preference IN ('auto_charge', 'invoice')),
  stripe_customer_id       TEXT,
  stripe_payment_method_id TEXT,
  stripe_setup_session_id  TEXT,
  balance                  NUMERIC(10,2) DEFAULT 0,  -- running tab (resets on payment)
  status                   TEXT DEFAULT 'pending_setup'
                             CHECK (status IN ('active', 'frozen', 'pending_setup')),
  freeze_reason            TEXT,
...
-- All transactions: purchases, payments, adjustments
CREATE TABLE IF NOT EXISTS school_transactions (
  id                       UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  account_id               UUID REFERENCES school_accounts(id) ON DELETE SET NULL,
  type                     TEXT NOT NULL
                             CHECK (type IN (
                               'purchase',        -- account tab purchase
                               'card_sale',       -- one-off card payment (no account)
                               'cash_sale',       -- one-off cash payment
                               'payment',         -- parent paid down their balance
                               'adjustment',      -- admin manual adjustment
                               'billing_charge',  -- Friday auto-charge success
                               'billing_invoice'  -- Friday invoice sent
                             )),
  amount                   NUMERIC(10,2) NOT NULL,  -- positive = charge, negative = credit
  description              TEXT,
...
-- Coffee shop menu (managed by admin)
...
CREATE INDEX IF NOT EXISTS school_accounts_token_idx     ON school_accounts(registration_token);
