...
// Sunday 10 AM before the dinner's week
function getRevealForDinner(dinner: DinnerMenu) {
  const d = new Date(dinner.date + "T12:00:00");
  const daysSinceSunday = d.getDay(); // Mon=1, Tue=2, Thu=4
...
export default function DinnerPage() {
  ...
  // Countdown to the next open cutoff
  ...
  if (loading) {
    return (
      <main className="bg-black text-white min-h-screen flex items-center justify-center">
        <p className="text-zinc-400">Loading this week&apos;s dinners...</p>
      </main>
    );
  }
