...
export async function POST(req: NextRequest) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    // ── Coop Celebration orders — handled by /api/coopcelebrate/success ──────
    if (meta.order_type === "coopcelebrate") {
      console.log(`Celebration order session ${session.id} — skipping regular order insert`);
      return NextResponse.json({ received: true });
    }
